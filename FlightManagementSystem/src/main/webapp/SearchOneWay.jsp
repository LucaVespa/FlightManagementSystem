...
<html>
	...
	<body>
		...
		try {
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT * FROM FlightDetails WHERE DATE(depDate) BETWEEN ? AND ? ORDER BY DATE(depDate)");
			
			DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
			LocalDate start = LocalDate.parse(depTime.trim(), dateFormatter).minusDays(flexibility);
			ps1.setString(1, start.toString());

			LocalDate end = LocalDate.parse(depTime.trim(), dateFormatter).plusDays(flexibility);
			ps1.setString(2, end.toString());
